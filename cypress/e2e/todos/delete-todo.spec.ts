describe("Delete Todo", () => {
  ...
  it("should delete todo when confirmed", () => {
    ...
    cy.get('h3').first().then(($title) => {
      ...
      cy.contains(todoTitle, { timeout: 15000 }).should("not.exist");
    });
  });
  ...
  it("should not delete todo when confirmation is cancelled", () => {
    ...
    cy.get('h3').first().then(($title) => {
      ...
      cy.contains(todoTitle, { timeout: 10000 }).should("be.visible");
    });
  });
  ...
  it("should delete multiple todos independently", () => {
    ...
    cy.get('h3').then(($titles) => {
      ...
      cy.window().then((win) => {
        ...
        if (win.confirm && typeof (win.confirm as any).restore === 'function') {
          (win.confirm as any).restore();
        }
        cy.stub(win, "confirm").returns(true);
      });
      cy.get('button').contains("Delete").first().click();
      cy.contains(firstTitle, { timeout: 15000 }).should("not.exist");
      ...
      cy.get('button').contains("Delete").first().click();
      cy.contains(secondTitle, { timeout: 15000 }).should("not.exist");
      ...
      cy.contains("No todos yet. Create your first todo above!", { timeout: 10000 }).should("be.visible");
    });
  });
  ...
  it("should show empty state after deleting all todos", () => {
    ...
    cy.contains("No todos yet. Create your first todo above!", { timeout: 15000 }).should("be.visible");
  });
});
